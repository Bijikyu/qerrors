...
### @qerrors/sanitization
**Purpose:** Comprehensive data sanitization for preventing sensitive information leakage in logs and outputs.

**Explanation:**  
This module provides enterprise-grade sanitization of sensitive data in log messages and context objects. It prevents information disclosure through comprehensive pattern matching for passwords, tokens, PII, credit cards, and other sensitive data. The module includes both string content sanitization and recursive object property sanitization, making it ideal for any application that logs user data or handles sensitive information. It's particularly valuable for applications in finance, healthcare, or any domain dealing with PII.

The module solves the critical security problem of sensitive data leakage in logs, which is a common compliance issue (GDPR, HIPAA, PCI-DSS). It would be reused across any Node.js application that needs to log data while maintaining security compliance, including web servers, API gateways, microservices, and background job processors.

Inputs: Raw strings, objects, arrays, and nested data structures containing potentially sensitive information.
Outputs: Sanitized versions with sensitive patterns redacted while preserving useful debugging information.

Important constraints: The module uses conservative patterns to avoid false negatives, includes performance optimizations for high-throughput logging scenarios, and handles edge cases like deeply nested objects and circular references.
