...
### @qerrors/input-validation
**Purpose:** Comprehensive security-focused input validation for preventing injection attacks and data corruption.

**Explanation:**  
This module provides enterprise-grade input validation with comprehensive security patterns to prevent XSS, SQL injection, command injection, path traversal, and other attack vectors. It includes type validation, length limits, pattern matching, and encoding validation with configurable rules for different input types. The module is particularly valuable for web applications, APIs, and any system that processes user input or external data.

The module solves the critical security problem of input validation, which is a primary defense against injection attacks. It would be reused across any Node.js application that needs to validate and sanitize input data securely, including web servers, API gateways, microservices, and data processing pipelines.

Inputs: Raw strings, objects, arrays, and JSON payloads containing potentially malicious or malformed data.
Outputs: Validation results with sanitized data, error messages, and security warnings.

Important constraints: The module uses conservative security patterns, includes comprehensive regex-based detection for attack vectors, and handles edge cases like encoded attacks and prompt injection for AI systems.
