...
describe('Express.js Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Add qerrors middleware for error handling
  ...
  describe('JSON Error Responses', () => {
    test('should return JSON error for API requests', async () => {
      ...
      expect(response.body.error).toHaveProperty('message', 'API test error');
      expect(response.body.error).toHaveProperty('uniqueErrorName');
      expect(response.body.error).toHaveProperty('timestamp');
    });

    test('should handle validation errors', async () => {
      app.post('/api/validate', (req, res, next) => {
        const error = qerrors.createTypedError('Invalid input', qerrors.ErrorTypes.VALIDATION, 'VALIDATION_ERROR');
        next(error);
      });
  ...
  describe('HTML Error Responses', () => {
    ...
    test('should escape HTML entities in error messages', async () => {
      app.get('/html/escape', (req, res, next) => {
        const error = new Error('Error with <strong>HTML</strong> & "quotes"');
        next(error);
      });
  ...
  describe('Controller Error Handling', () => {
    test('should handle controller errors gracefully', async () => {
      app.post('/controller/error', async (req, res) => {
        try {
          throw new Error('Controller operation failed');
        } catch (error) {
          await qerrors.handleControllerError(res, error, 'testController', req.body, 'User-friendly message');
        }
      });
      ...
      expect(response.status).toBe(500);
      expect(response.body).toHaveProperty('message', 'User-friendly message');
    });

    test('should handle authentication errors', async () => {
      app.post('/auth/login', async (req, res) => {
        try {
          throw qerrors.createTypedError('Invalid credentials', qerrors.ErrorTypes.AUTHENTICATION, 'AUTH_FAILED');
        } catch (error) {
          await qerrors.handleControllerError(res, error, 'authController', req.body);
        }
      });
  ...
  describe('Error Severity Integration', () => {
    test('should handle critical errors', async () => {
      app.get('/critical', async (req, res, next) => {
        ...
        await qerrors.logErrorWithSeverity(error, 'criticalOperation', req.query, qerrors.ErrorSeverity.CRITICAL);
        next(error);
      });
