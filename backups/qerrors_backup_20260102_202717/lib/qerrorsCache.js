...
const startAdviceCleanup = () => {
    if (CACHE_TTL_SECONDS === 0 || ADVICE_CACHE_LIMIT === 0 || cleanupHandle)
        return;
...
const stopAdviceCleanup = () => {
    ...
    // Legacy variables will be null after clearIntervalAndNull operations
    cleanupHandle = null;
    autoTuningHandle = null;
};
...
const purgeExpiredAdvice = () => {
    if (CACHE_TTL_SECONDS === 0 || ADVICE_CACHE_LIMIT === 0)
        return;
    adviceCache.purgeStale();
    adviceCache.size === 0 && stopAdviceCleanup();
};
...
//# sourceMappingURL=qerrorsCache.js.map