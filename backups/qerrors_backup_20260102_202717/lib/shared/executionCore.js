 ...
 */
export const safeRun = (name, fn, fallback, info) => {
    try {
        return fn();
    }
    catch (err) {
 ...
 */
export const executeWithErrorHandling = async (options) => {
    ...
    try {
        ...
        if (logMessage || failureMessage) {
            ...
            try {
                ...
                await logger.logError(message, { ...errorContext, error });
            }
            catch (logErr) {
 ...
 */
export const executeWithQerrors = async (options) => {
