 ...
 */
const getMissingEnvVars = varArr => { varArr.forEach(name => { if (!process.env[name]) {
    logError(new Error(`Missing environment variable: ${name}`), 'envUtils.getMissingEnvVars', { operation: 'environment_variable_check', variableName: name });
