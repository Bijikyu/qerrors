...
async detectBreach(incidentData) {
  try {
    ...
    qerrors(error, 'breachNotificationService.detectBreach', {
      operation: 'breach_detection_assessment',
      hasIncidentData: !!incidentData,
      incidentType: incidentData?.type
    });
    throw new Error('Failed to detect and assess breach');
  }
}
...
async assessRisk(incidentData) {
  try {
    ...
    qerrors(error, 'breachNotificationService.assessRisk', {
      operation: 'breach_risk_assessment',
      hasIncidentData: !!incidentData,
      dataTypesCount: incidentData?.dataTypes?.length || 0
    });
