 ...
 * }
 */
class CircuitBreakerWrapper{constructor(operation,serviceName,options){if(!options||options.failureThreshold<=0)throw new Error('failureThreshold must be positive');if(options.recoveryTimeoutMs<=0)throw new Error('recoveryTimeoutMs must be positive');this.operation=operation;this.serviceName=serviceName;this.options=options;const opossumOptions={timeout:options.timeoutMs||10000,errorThreshold:options.failureThreshold,resetTimeout:options.recoveryTimeoutMs,rollingCountTimeout:options.monitoringPeriodMs||60000,rollingCountBuckets:10,cacheEnabled:false,enabled:true};this.breaker=new CircuitBreaker(operation,opossumOptions);this._setupEventListeners();this._initializeMetrics();}

  /**
   * Setup opossum event listeners for logging and monitoring
   * @private
   */
  _setupEventListeners(){this.breaker.on('open',()=>console.log(`[CircuitBreaker] ${this.serviceName}: transitioning to OPEN`));this.breaker.on('halfOpen',()=>console.log(`[CircuitBreaker] ${this.serviceName}: transitioning to HALF_OPEN`));this.breaker.on('close',()=>console.log(`[CircuitBreaker] ${this.serviceName}: transitioning to CLOSED`));this.breaker.on('failure',(result,error)=>{try{const utils=require('./utils');utils.logError(error,`CircuitBreaker-${this.serviceName}`,{state:this.getState(),result});}catch{}});this.breaker.on('success',(result)=>{});this.breaker.on('timeout',()=>console.warn(`[CircuitBreaker] ${this.serviceName}: operation timeout`));}
...
const CircuitState={CLOSED:'CLOSED',OPEN:'OPEN',HALF_OPEN:'HALF_OPEN'};module.exports={CircuitBreakerWrapper,CircuitState,createCircuitBreaker};