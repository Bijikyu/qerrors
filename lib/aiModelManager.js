...
const createLangChainModel = (provider = MODEL_PROVIDERS.GOOGLE, modelName = null) => {
    ...
    if (!providerConfig) throw new Error(`Unsupported AI provider: ${provider}`);
    const missingVars = providerConfig.requiredEnvVars.filter(envVar => !process.env[envVar]);
    if (missingVars.length > 0) throw new Error(`Missing required environment variables for ${provider}: ${missingVars.join(', ')}`);
    const selectedModel = modelName || providerConfig.defaultModel, modelConfig = providerConfig.models[selectedModel];
    if (!modelConfig) throw new Error(`Unsupported model ${selectedModel} for provider ${provider}`);
    switch (provider) {
...
class AIModelManager {
    constructor() {
        this.currentProvider = process.env.QERRORS_AI_PROVIDER || MODEL_PROVIDERS.GOOGLE;
...
const getAIModelManager = () => {
  if (!aiModelManager) aiModelManager = new AIModelManager();
  return aiModelManager;
};

/**
 * Reset the AI Model Manager (useful for reconfiguration)
 */
const resetAIModelManager = () => { aiModelManager = null; };

module.exports = { AIModelManager, getAIModelManager, resetAIModelManager, MODEL_PROVIDERS, MODEL_CONFIGS, createLangChainModel };