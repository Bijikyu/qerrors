...
switch(this.currentProvider){case MODEL_PROVIDERS.OPENAI:return new ChatOpenAI({modelName:selectedModel,temperature:1,maxTokens:parseInt(process.env.QERRORS_MAX_TOKENS)||modelConfig.maxTokens,topP:1,frequencyPenalty:0,presencePenalty:0,openAIApiKey:process.env.OPENAI_API_KEY,verbose:process.env.QERRORS_VERBOSE==='true',modelKwargs:{response_format:{type:'json_object'}}});
case MODEL_PROVIDERS.GOOGLE:return new ChatGoogleGenerativeAI({modelName:selectedModel,temperature:1,maxOutputTokens:parseInt(process.env.QERRORS_MAX_TOKENS)||modelConfig.maxTokens,topP:1,apiKey:process.env.GEMINI_API_KEY,verbose:process.env.QERRORS_VERBOSE==='true'});
