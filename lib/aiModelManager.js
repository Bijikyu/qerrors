...
class AIModelManager {
  constructor() {
    ...
    this.analysisModelCache = new Map(); // Cache for analysis model instances
    this.maxCacheSize = 10; // Maximum number of cached model instances
    this.cacheAccessOrder = []; // Track access for LRU eviction
  ...
  createAnalysisModel() {
    ...
    if (this.analysisModelCache.has(cacheKey)) {
      ...
      return this.analysisModelCache.get(cacheKey);
    }
    ...
    if (this.analysisModelCache.size >= this.maxCacheSize) {
      // Remove least recently used entry
      const oldestKey = this.cacheAccessOrder.shift();
      if (oldestKey) {
        this.analysisModelCache.delete(oldestKey);
      }
    }
