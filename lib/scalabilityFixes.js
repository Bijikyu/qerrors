...
class ScalableErrorHandler {
  ...
  addToHistory(errorRecord) {
     ...
     // Enforce size limit immediately to prevent unbounded growth
  ...
  enforceHistoryLimit(customLimit = null) {
     ...
     if (this.errorHistory.length > limit) {
       // Remove oldest entries efficiently
       const removeCount = this.errorHistory.length - limit;
     ...
     // Additional safety check to prevent memory bloat
     if (this.errorHistory.length > limit * 1.5) {
       console.warn(`Error history exceeded safe limit (${this.errorHistory.length} > ${limit * 1.5}), forcing emergency cleanup`);
       this.errorHistory.splice(0, Math.floor(this.errorHistory.length / 2));
     }
   }
