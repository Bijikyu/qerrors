...
const memoryMonitor = {
  ...
  getMemoryPressure: () => {
    ...
    const heapUsedPercent = heapTotal > 0 ? (heapUsed / heapTotal) * 100 : 0;
...
const calculateMemoryAwareCacheSize = () => {
  const baseLimit = Math.min(ADVICE_CACHE_LIMIT || 50, 200), memoryPressure = memoryMonitor.getMemoryPressure();
  
  switch (memoryPressure) {
    case 'critical': return Math.max(5, Math.floor(baseLimit * 0.1));
    case 'high': return Math.max(10, Math.floor(baseLimit * 0.25));
    case 'medium': return Math.max(20, Math.floor(baseLimit * 0.5));
    default: return Math.min(baseLimit, 100);
  }
};
