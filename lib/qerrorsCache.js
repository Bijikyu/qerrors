 ...
 */
// Size cache to avoid repeated JSON.stringify operations with memory limits
const sizeCache = new LRUCache({
  max: 50, // Reduced cache size to prevent memory bloat
  ttl: 5 * 60 * 1000, // 5 minutes
  maxSize: 1024 * 1024, // 1MB max total cache size
  updateAgeOnGet: true
});
  ...
  return new Promise((resolve, reject) => {
    ...
    setImmediate(() => {
      try {
        ...
        // Early size validation to prevent memory bloat
        const MAX_ADVICE_SIZE = 50 * 1024; // 50KB limit per advice object
        
        // Create a unique hash key for the advice object
        let hashKey;
        let quickSize = 0; // Initialize to prevent reference errors
        
        if (typeof advice === 'object' && advice !== null) {
          // Quick size check before expensive operations with timeout protection
          quickSize = JSON.stringify(advice).length;
          if (quickSize > MAX_ADVICE_SIZE) {
            resolve(MAX_ADVICE_SIZE); // Return max size for oversized objects
            return;
          }
        ...
        // Calculate size asynchronously with chunked processing for large objects
        const processChunked = () => {
          try {
            const size = JSON.stringify(advice).length;
            // Only cache if within reasonable size limits
            if (size <= MAX_ADVICE_SIZE) {
        ...
        // Use setImmediate with additional yielding for large objects
        if (quickSize > 1024) {
          setImmediate(processChunked);
        } else {
          processChunked();
        }
      } catch (err) {
