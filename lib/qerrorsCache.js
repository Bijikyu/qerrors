...
const setAdviceInCache = (key, advice) => {
  ...
  if (ADVICE_CACHE_LIMIT !== 0) {
    ...
    const adviceSize = JSON.stringify(advice).length;
    const maxEntrySize = 50 * 1024; // 50KB max per entry
    
    if (adviceSize > maxEntrySize) {
      console.warn(`Cache entry too large (${adviceSize} bytes), skipping cache for key: ${key}`);
      return;
    }
