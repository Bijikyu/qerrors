...
/**
 * Simplified JSON.stringify without caching to prevent memory leaks
 * Caching was causing memory issues, disabled for production stability
...
class AdaptiveSocketPoolManager {
  constructor() {
    ...
    this.adjustmentInProgress = false; // Prevent concurrent adjustments
...
/**
   * Add load measurement to history with efficient running sum maintenance
   */
  addLoadMeasurement(load) {
    ...
    // Remove oldest value from running sum if at limit
  ...
  /**
   * Get average load over recent history using running average for efficiency
   */
  getAverageLoad() {
    if (this.loadHistory.length === 0) return 0;
    
    // Use running sum to avoid array iteration
    if (this.runningSum === undefined) {
      // Initialize running sum on first call
      try {
  ...
  /**
   * Adjust agent configurations if needed (non-blocking)
   */
  adjustAgents() {
    ...
    // Schedule adjustment in background to prevent blocking
    setImmediate(() => {
      try {
        ...
        // Create new agents with updated settings in background
        ...
        agentCreationPromise
          .then(({ newHttpAgent, newHttpsAgent }) => {
            // Schedule agent switching in next tick to prevent blocking
            setImmediate(() => {
              try {
                // Destroy old agents gracefully
                ...
                // Replace with new agents atomically
...
const axiosInstance = axios.create({
 // Use adaptive socket pool agents for dynamic connection optimization
  ...
  // Size limits to prevent cost overruns and resource exhaustion (reduced to 500KB)
  maxContentLength: 512 * 1024, // 512KB max response size
  maxBodyLength: 512 * 1024    // 512KB max request body size
});
