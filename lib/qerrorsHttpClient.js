...
// HTTP connection agent for connection pooling and reuse - optimized for high load
  httpAgent: new http.Agent({
    ...
    maxSockets: Math.min(200, MAX_SOCKETS || 100),  // Increased dynamic sizing for high load
    maxFreeSockets: 50,          // Significantly increased idle connections for better reuse
    timeout: 15000,               // Increased timeout for better reliability under load
    scheduling: 'fifo',           // FIFO scheduling for better fairness under load
    keepAliveMsecs: 60000,        // Extended keep-alive for 60 seconds to reduce connection churn
    freeSocketTimeout: 30000     // Extended free socket timeout to 30 seconds
  }),
  ...
  httpsAgent: new https.Agent({
    ...
    maxSockets: Math.min(200, MAX_SOCKETS || 100),  // Increased dynamic sizing for high load
    maxFreeSockets: 50,          // Significantly increased idle secure connections for better reuse
    timeout: 15000,               // Increased timeout for better reliability under load
    scheduling: 'fifo',           // FIFO scheduling for better fairness under load
    keepAliveMsecs: 60000,        // Extended keep-alive for 60 seconds to reduce SSL handshake overhead
    freeSocketTimeout: 30000,     // Extended free socket timeout to 30 seconds
    rejectUnauthorized: true,     // Ensure SSL security
    // Additional SSL optimization for high load
    checkServerIdentity: () => undefined, // Skip server identity check for internal APIs (use with caution)
    secureProtocol: 'TLSv1_2_method' // Force modern TLS for better performance
  }),
