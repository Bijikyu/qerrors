              ...
              const estimateSizeAsync = (data, callback) => {
                setImmediate(() => {
                  try {
                    ...
                    callback(null, jsonString.length);
                  } catch (e) {
                ...
                if (sizeEstimate > 51200) { // 50KB threshold
                  ...
                  setImmediate(() => {
                    try {
                      ...
                      // Use chunked JSON processing for very large objects
                      setImmediate(() => {
                        try {
                          const jsonString = JSON.stringify(data);
                          ...
                          const processChunk = () => {
                            const endIndex = Math.min(i + chunkSize, jsonString.length);
                            const chunk = jsonString.slice(i, endIndex);
                            hash.update(chunk);
                            i = endIndex;
                            
                            if (i < jsonString.length) {
                              // Yield control before processing next chunk
