...
/**
 * Qerrors HTTP Client Module
 * 
 * Provides optimized HTTP client functionality for AI API communication
 * with adaptive connection pooling, retry logic, and error handling.
 * Implements intelligent socket management for high-load scenarios.
 */
...
const { safeJsonStringify } = require('./shared/jsonHelpers');

const cachedJsonStringify = safeJsonStringify;
const { MAX_SOCKETS, MAX_FREE_SOCKETS } = localVars;

/**
 * Manages adaptive socket pools for HTTP connections
 * Dynamically adjusts pool sizes based on load and performance metrics
 */
class AdaptiveSocketPoolManager {
  constructor() {
    ...
    this.adjustmentInterval = 30000;
    this.lastAdjustment = Date.now();
    this.httpAgent = null;
    this.httpsAgent = null;
    this.adjustmentInProgress = false;
    this.adjustmentIntervalId = null; // Track for cleanup
  ...
  /**
   * Initializes HTTP and HTTPS agents with current configuration
   */
  initializeAgents() {
...
/**
   * Adds a load measurement to the history
   * @param {number} load - Load measurement (0-1)
   */
  addLoadMeasurement(load) {
  if (this.runningSum === undefined) {
    this.runningSum = 0;
  }
  
  if (this.loadHistory.length >= this.maxLoadHistory) {
    this.runningSum -= this.loadHistory.shift();
  }
  
  this.loadHistory.push(load);
  this.runningSum += load;
}
  
/**
   * Updates current load based on active requests and socket capacity
   */
updateLoad() {
  const maxSockets = this.httpAgent?.maxSockets || this.baseMaxSockets || 1;
  const activeRequests = typeof pendingRequests?.size === 'number' ? pendingRequests.size : 0;
  const loadSample = Math.max(0, Math.min(1, activeRequests / Math.max(1, maxSockets)));
  this.addLoadMeasurement(loadSample);
  this.currentLoad = this.getAverageLoad();
}
  
/**
   * Calculates average load from history
   * @returns {number} Average load (0-1)
   */
  getAverageLoad() {
  ...
  return this.runningSum / this.loadHistory.length;
}
  
/**
   * Calculates current maximum sockets based on average load
   * @returns {number} Maximum sockets count
   */
...
adjustAgents() {
  ...
  setImmediate(() => {
    try {
      ...
      const agentCreationPromise = new Promise((resolve, reject) => {
        setImmediate(() => {
          try {
            ...
            resolve({ newHttpAgent, newHttpsAgent });
          } catch (error) {
            reject(error);
          }
        });
      ...
      agentCreationPromise
        .then(({ newHttpAgent, newHttpsAgent }) => {
          setImmediate(() => {
            try {
              ...
              console.error('Error during agent switching:', error.message);
            } finally {
              this.adjustmentInProgress = false;
            }
          });
        ...
        .catch((error) => {
          console.error('Error creating new socket pool agents:', error.message);
          this.adjustmentInProgress = false;
        });
          
    } catch (error) {
      console.error('Error in socket pool adjustment process:', error.message);
      this.adjustmentInProgress = false;
    }
  });
}
...
destroy() {
  try {
    // Clear adaptive adjustment interval
...
const axiosInstance = axios.create({
  ...
  timeout: config.getInt('QERRORS_TIMEOUT', 15000),
  headers: {
...
/**
  ...
  async function postWithRetry(url, data, opts, capMs) {
    try {
      ...
      // Check size limit after adding
      if (pendingRequests.size > MAX_PENDING_REQUESTS) {
        // Remove oldest pending request (first one in Map)
        const oldestKey = pendingRequests.keys().next().value;
        if (oldestKey) {
          pendingRequests.delete(oldestKey);
          // Log the removal to track potential request loss
          qerrors(new Error('Pending request evicted due to size limit'), 'qerrorsHttpClient.pendingRequestEviction', {
  ...
  async function createRequestKeyAsync(url, data) {
    try {
      ...
      return new Promise((resolve, reject) => {
        // Use multiple setImmediate calls for large objects to prevent blocking
        const processJsonAsync = (data, callback) => {
          setImmediate(() => {
            try {
              if (!data || typeof data !== 'object') {
                // Simple case - non-objects can be processed quickly
                callback(null, JSON.stringify(data));
                return;
              }
              
              // For objects, estimate size first asynchronously to avoid blocking
              const estimateSizeAsync = (data, callback) => {
                setImmediate(() => {
                  try {
                    // Use streaming JSON.stringify to avoid blocking on large objects
                    ...
                    // Create a readable stream from cached JSON.stringify output
                    const jsonString = cachedJsonStringify(data);
                    totalLength = jsonString.length;
                    callback(null, totalLength);
                  } catch (e) {
                    // If stringify fails, fall back to object inspection
                    const sizeEstimate = Object.keys(data).length * 20; // Rough estimate
                    callback(null, sizeEstimate);
                  }
                });
              };
              
              estimateSizeAsync(data, (error, sizeEstimate) => {
                if (error) {
                  callback(error);
                  return;
                }
                
                if (sizeEstimate > 51200) { // 50KB threshold
                  // For large objects, use streaming hash to avoid memory spikes
                  setImmediate(() => {
                    try {
                      const crypto = require('crypto');
                      const hash = crypto.createHash('sha256');
                      
// Use streaming JSON processing for very large objects to avoid blocking
                          setImmediate(() => {
                            try {
                              const crypto = require('crypto');
                              const hash = crypto.createHash('sha256');
                              
// Stream cached JSON.stringify output directly to hash
                               const jsonString = cachedJsonStringify(data);
                              
                              // Process in smaller chunks with better yielding
                              const chunkSize = 4096; // 4KB chunks for better responsiveness
                              let offset = 0;
                              ...
                              const processChunk = () => {
                                ...
                                const end = Math.min(offset + chunkSize, jsonString.length);
                                const chunk = jsonString.slice(offset, end);
                                hash.update(chunk);
                                offset = end;
                                
                                // Use nextTick instead of setImmediate for faster processing
                  ...
                  // Small objects - process in next tick but still async with cached stringify
                  setImmediate(() => {
                    try {
                      callback(null, cachedJsonStringify(data));
                    } catch (e) {
                      callback(e);
                    }
                  });
        ...
        processJsonAsync(data, (error, jsonString) => {
          if (error) {
...
class CircuitBreaker {
  ...
  updateResponseTimeHistory(responseTime) {
     ...
     // Update average response time metric with non-blocking calculation for large histories
     const historySize = this.responseTimeHistory.length;
     if (historySize <= 20) {
       // Small history - calculate synchronously
       const avgTime = this.responseTimeHistory.reduce((sum, entry) => sum + entry.time, 0) / historySize;
       this.metrics.avgResponseTime = Math.round(avgTime);
     } else {
       // Large history - calculate in background to prevent blocking
       setImmediate(() => {
         try {
           ...
           // Ignore calculation errors to prevent circuit breaker failures
           console.warn('Error calculating response time average:', error.message);
         }
       });
     }
   }
...
adjustThresholds() {
     ...
     // Schedule threshold adjustment in background to prevent blocking
     setImmediate(() => {
       try {
         ...
         // Ignore threshold adjustment errors to prevent circuit breaker failures
         console.warn('Error adjusting circuit breaker thresholds:', error.message);
       }
     });
   }
...
// Optimized LRU eviction with bounded sorting to prevent memory bloat
        const maxSortSize = 100; // Limit sort size for performance
        const entries = Array.from(responseCacheAccess.entries());
        
        if (entries.length <= maxSortSize) {
          // Small cache - full sort
          ...
          // Large cache - use partial selection with heap algorithm
          const heapSize = Math.min(entriesToRemove, maxSortSize);
          const heap = [];
          
          // Build min-heap of smallest access times
          ...
          // Extract and delete entries
          while (heap.length > 0) {
            const [key] = heap.shift();
            responseCache.delete(key);
            responseCacheAccess.delete(key);
            if (heap.length > 0) {
...
startResponseCacheCleanup();
  ...
  /**
   * Process a single group of requests with batching
   */
  async function processRequestGroup(group, optimalBatchSize) {
    const results = [];
    ...
    try {
      ...
      try {
        ...
        for (const group of groupedRequests) {
          const groupResults = await processRequestGroup(group, optimalBatchSize);
...
function cleanupCache() {
  ...
  if (responseCache.size > MAX_RESPONSE_CACHE_SIZE) {
    ...
    // Optimized LRU eviction with bounded sorting (same optimization as above)
    const maxSortSize = 100;
    const entries = Array.from(responseCacheAccess.entries());
    ...
    if (entries.length <= maxSortSize) {
      ...
      // Large cache - use partial selection to avoid full sort
      const heapSize = Math.min(entriesToRemove, maxSortSize);
      const heap = [];
...
// Add process exit handlers to prevent memory leaks
const setupExitHandlers = () => {
  const cleanup = () => {
    console.info('Cleaning up HTTP client resources...');
    shutdown();
  };
...
module.exports = {
  ...
  cachedJsonStringify, // Performance optimization for JSON serialization
