...
/**
  ...
  /**
   * Internal function to execute request with retry logic and circuit breaking
   */
...
class CircuitBreaker {
  ...
  async execute(operation) {
    ...
    if (this.state === 'OPEN') {
      ...
      if (timeSinceLastFailure > adaptiveRecoveryTime) {
        ...
        throw new Error(`Circuit breaker is OPEN - preventing cascading failures. Recovery in ${Math.ceil((adaptiveRecoveryTime - timeSinceLastFailure) / 1000)}s`);
      }
    }
  ...
  /**
   * Update response time history for adaptive thresholds
   */
  updateResponseTimeHistory(responseTime) {
    ...
    const avgTime = this.responseTimeHistory.reduce((sum, entry) => sum + entry.time, 0) / this.responseTimeHistory.length;
  ...
  updateErrorRateHistory(error) {
    ...
    if (this.errorRateHistory.length > 50) {
      this.errorRateHistory = this.errorRateHistory.slice(-50);
    }
  }
