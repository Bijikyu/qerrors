...
class EnhancedRateLimiter {
  constructor(options = {}) {
    ...
    this.maxUserAgentCacheSize = 1000;
  ...
  /**
   * Hash user agent for better key distribution with caching
   */
  hashUserAgent(userAgent) {
    ...
    if (this.userAgentHashCache.size >= this.maxUserAgentCacheSize) {
      ...
      const firstKey = this.userAgentHashCache.keys().next().value;
      this.userAgentHashCache.delete(firstKey);
    }
  ...
  /**
   * Clean up endpoint statistics using LRU eviction
   */
  cleanupEndpointStats() {
    try {
      const maxEndpoints = 1000; // Limit to 1000 endpoints
      ...
      // Sort endpoints by last hit time
      const endpointsByAge = Object.entries(this.stats.endpointHits)
        .map(([endpoint, hits]) => ({
          endpoint,
          hits,
