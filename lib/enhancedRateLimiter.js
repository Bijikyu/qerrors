...
class EnhancedRateLimiter {
  constructor(options = {}) {
    ...
    // Track endpoint hit timestamps for LRU cleanup with strict size limits
    this.endpointHitTimes = new Map();
    this.maxEndpointStats = 200; // Reduced limit for memory efficiency
    this.maxEndpointHitTimes = 300; // Separate limit for hit timestamps
    
    // Cache for user agent hashes to avoid repeated computation - proper LRU
    this.userAgentHashCache = new Map();
    this.maxUserAgentCacheSize = 100; // Further reduced size for memory efficiency
    
    // Simple LRU implementation using Map (maintains insertion order)
    class SimpleLRU {
    ...
    this.endpointLRU = new SimpleLRU(this.maxEndpointHitTimes);
      ...
      store: {
        increment: async (key) => {
          try {
            ...
            // Update endpoint statistics with proper LRU management
            let endpointHits = this.stats.endpointHits[endpointPath];
            if (endpointHits === undefined) {
              endpointHits = 0;
              
              // Check if we need to evict old endpoints using proper LRU
            ...
            this.stats.endpointHits[endpointPath] = endpointHits + 1;
            
            // Track endpoint hit time using proper LRU
            this.endpointLRU.set(endpointPath, Date.now());
   ...
   * Always async to prevent blocking in request path
   */
  async hashUserAgent(userAgent) {
    ...
    // Use optimized hashing with memory monitoring and cleanup
    return new Promise(resolve => {
      // Add memory monitoring before expensive operations
      const memUsage = process.memoryUsage();
      const MAX_HEAP_USAGE = 100 * 1024 * 1024; // 100MB threshold
      
      if (memUsage.heapUsed > MAX_HEAP_USAGE) {
        // Trigger cleanup if memory usage is high
        this.userAgentLRU.clear();
      ...
      setImmediate(() => {
        try {
          // Use built-in crypto hash with size limits
          const crypto = require('crypto');
          const MAX_UA_LENGTH = 500; // Limit user agent length for hashing
          const truncatedUA = userAgent.length > MAX_UA_LENGTH ? 
            userAgent.substring(0, MAX_UA_LENGTH) : userAgent;
          
          const hash = crypto.createHash('md5').update(truncatedUA).digest('hex');
          const hashResult = hash.substring(0, 8);
          
          // Cache with size monitoring
          if (this.userAgentLRU.size < 1000) { // Limit cache size
            this.userAgentLRU.set(userAgent, hashResult);
          }
          
          resolve(hashResult);
        } catch (error) {
          // Fallback to simple hash on error with minimal allocation
          const fallbackHash = Math.abs(userAgent.length + userAgent.charCodeAt(0)).toString(36);
          if (this.userAgentLRU.size < 1000) {
            this.userAgentLRU.set(userAgent, fallbackHash);
          }
