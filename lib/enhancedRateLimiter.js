...
class EnhancedRateLimiter {
  ...
  async hashUserAgent(userAgent) {
    ...
    // Use optimized hashing with crypto for better performance
    return new Promise(resolve => {
      setImmediate(() => {
        try {
          // Use built-in crypto hash for better performance and security
          const crypto = require('crypto');
          const hash = crypto.createHash('md5').update(userAgent).digest('hex');
          const hashResult = hash.substring(0, 8); // Use first 8 chars for brevity
          
          // Cache using proper LRU
