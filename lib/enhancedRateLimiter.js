...
class EnhancedRateLimiter {
  constructor(options = {}) {
    ...
    // Statistics for monitoring with bounded storage
    this.stats = {
      totalRequests: 0,
      blockedRequests: 0,
      endpointHits: {},
    ...
    // Track endpoint hit timestamps for LRU cleanup with size limit
    this.endpointHitTimes = new Map();
    this.maxEndpointStats = 500; // Limit endpoint statistics to prevent memory bloat
    ...
    this.userAgentAccessOrder = []; // Use array for O(1) LRU operations
      ...
      store: {
        increment: async (key) => {
          try {
            ...
            // Update endpoint statistics with async LRU eviction to prevent blocking
            if (!this.stats.endpointHits[endpointPath]) {
              // Check if we need to evict old endpoints
              const endpointCount = Object.keys(this.stats.endpointHits).length;
              if (endpointCount >= this.maxEndpointStats) {
                // Schedule LRU eviction in background to prevent blocking request path
                setImmediate(() => {
                  try {
                    // Find least recently used endpoint to evict (with iteration limit)
                    let oldestEndpoint = null;
                    let oldestTime = Date.now();
                    let iterations = 0;
                    const maxIterations = Math.min(50, this.endpointHitTimes.size);
                    
                    for (const [endpoint, time] of this.endpointHitTimes.entries()) {
                      if (time < oldestTime) {
                        oldestTime = time;
                    ...
                    if (oldestEndpoint && oldestEndpoint !== endpointPath) {
                      delete this.stats.endpointHits[oldestEndpoint];
                      this.endpointHitTimes.delete(oldestEndpoint);
                    }
                  } catch (error) {
                    // Ignore eviction errors to prevent rate limiter failures
                    console.warn('Error during endpoint LRU eviction:', error.message);
                  }
                });
              }
            ...
            // Track endpoint hit time for LRU cleanup (async)
  ...
  async hashUserAgent(userAgent) {
    ...
    if (this.userAgentHashCache.has(userAgent)) {
      ...
      const index = this.userAgentAccessOrder.indexOf(userAgent);
      if (index > -1) {
        this.userAgentAccessOrder.splice(index, 1);
        this.userAgentAccessOrder.push(userAgent);
      }
      return this.userAgentHashCache.get(userAgent);
    }
    
    // Compute hash with chunked processing for long strings
    let hash = 0;
    const userAgentLength = userAgent.length;
    
    // For very long user agents, process in chunks to avoid blocking
    if (userAgentLength > 1000) {
      // Process in chunks using setImmediate for non-blocking
      return new Promise(resolve => {
        let currentIndex = 0;
        const chunkSize = 100;
        let hash = 0; // Initialize hash inside Promise to fix closure issue
        
        const processChunk = () => {
          const endIndex = Math.min(currentIndex + chunkSize, userAgentLength);
          ...
          if (currentIndex < userAgentLength) {
            ...
            // Cache with O(1) LRU eviction - check before adding new entry
      ...
      // Synchronous processing for short strings
    ...
    setInterval(() => {
      try {
        ...
        if (this.stats.totalRequests > 100000) {
          ...
          this.userAgentAccessOrder.length = 0; // Clear array efficiently
          this.stats.lastReset = Date.now();
  ...
  /**
   * Clean up user agent cache using non-blocking LRU eviction
   */
  cleanupUserAgentCache() {
    try {
      const cacheSize = this.userAgentHashCache.size;
      if (cacheSize <= this.maxUserAgentCacheSize) {
        return; // No cleanup needed
      }
      
      // Schedule cleanup in background to prevent blocking
      setImmediate(() => {
        try {
          const excess = cacheSize - this.maxUserAgentCacheSize;
          let evicted = 0;
          
          // Evict oldest entries using access order array
          for (let i = 0; i < Math.min(excess, this.userAgentAccessOrder.length) && evicted < excess; i++) {
            const keyToRemove = this.userAgentAccessOrder[i];
            if (keyToRemove && this.userAgentHashCache.has(keyToRemove)) {
              this.userAgentHashCache.delete(keyToRemove);
              evicted++;
            }
          }
          
          // Update access order array
          this.userAgentAccessOrder.splice(0, evicted);
          
          if (evicted > 0) {
            console.debug(`Cleaned up ${evicted} expired user agent cache entries`);
          }
        } catch (error) {
          console.warn('Error during user agent cache cleanup:', error.message);
        }
      });
    } catch (error) {
      console.warn('Error in user agent cache cleanup setup:', error.message);
    }
  }
