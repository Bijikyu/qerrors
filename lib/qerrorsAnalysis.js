...
const generateErrorFingerprint = (message, stack) => {
  ...
  // Use a simple but effective hash algorithm (FNV-1a variant)
  // This is much faster than SHA-256 for cache key generation
  let hash1 = 2166136261; // FNV offset basis
  let hash2 = 2166136261; // Second hash for collision reduction
  
  // Hash the error message
  for (let i = 0; i < msg.length; i++) {
    hash1 ^= msg.charCodeAt(i);
    hash1 *= 16777619; // FNV prime
    hash1 |= 0; // Convert to 32-bit integer
    
    hash2 ^= msg.charCodeAt(i) * 31; // Different multiplier for variety
    hash2 *= 16777619;
    hash2 |= 0;
  }
  
  // Hash the stack trace with different starting values
  let hash3 = 2166136261;
  let hash4 = 2166136261;
  
  for (let i = 0; i < stk.length; i++) {
    hash3 ^= stk.charCodeAt(i) * 37; // Different multiplier
    hash3 *= 16777619;
    hash3 |= 0;
    
    hash4 ^= stk.charCodeAt(i) * 17; // Yet another multiplier
    hash4 *= 16777619;
    hash4 |= 0;
  }
  
  // Combine hashes for better distribution
  const combined1 = hash1 ^ (hash2 << 16) ^ (hash2 >>> 16);
  const combined2 = hash3 ^ (hash4 << 16) ^ (hash4 >>> 16);
  const finalHash = combined1 ^ combined2;
  
  // Convert to hexadecimal string
  // Ensure positive number and pad to consistent length
  const positiveHash = finalHash & 0x7fffffff;
