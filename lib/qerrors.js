...
async function analyzeError(error, context) {
	console.log(`qerrors error analysis is running with context: ${context}`); // Log the call details
	if (!process.env.OMNICHAT_TOKEN) {
		console.error("Missing OMNICHAT_TOKEN in environment variables.");
		return null;
	}
	...
	const requestBody = {
		...
		config: {
			token: process.env.OMNICHAT_TOKEN, // Use the environment variable for your OmniChat API key
			model: 'o3-mini'
		}
	};
	try {
		const response = await axios.post('https://omnichat.use-api.com/api/chatCompletion', requestBody);
		const advice = response.data || "No response received from API";
		console.log(`qerrors is returning advice for the error in ${context}:`);
		...
		console.error('Error in analyzeError function of qerrors:', axiosError.message); // Log any issues with the GPT API call
...
async function qerrors(error, context, req, res, next) {
	...
	console.log(`qerrors is running with context: ${context}`);
	...
	console.log(`qerrors has run`); // Logging completion
