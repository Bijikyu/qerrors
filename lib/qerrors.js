...
async function analyzeError(error, context) {
	...
	const requestBody = {
		...
		headers: {
			'x-api-key': process.env.OMNICHAT_TOKEN // Use the environment variable for your OmniChat API key
		}
	};
	try {
		...
		console.error(`Error in analyzeError function of qerrors for ${error.uniqueErrorName}:', ${axiosError.message}`); // Log any issues with the GPT API call
...
async function qerrors(error, context, req, res, next) {
	...
	const uniqueErrorName = "ERROR"+ error.name + Date.now() + "_" + Math.random().toString(36).slice(2);
