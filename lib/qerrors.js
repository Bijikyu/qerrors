...
const logger = require('./logger');          // Winston-based logging system - cached at module scope
...
// Global scalable error handler instance with memory-efficient settings
    ...
    if (!shutdownListenersAdded) {
      const gracefulShutdown = () => {
        if (scalableErrorHandler) {
          ...
          scalableErrorHandler = null; // Clear reference to prevent memory leaks
        }
      ...
      cleanupInterval = setInterval(() => {
        if (scalableErrorHandler) {
          try {
            // Trigger cleanup if handler has cleanup method
            if (typeof scalableErrorHandler.cleanup === 'function') {
              scalableErrorHandler.cleanup();
            }
          } catch (error) {
...
/**
 * Force cleanup of the error handler instance for memory management
 */
...
function logAsync(level, message) {
  ...
  setImmediate(() => {
    try {
      // Use pre-initialized logger to prevent blocking
...
async function qerrors(error, context, req, res, next) {
  ...
  // Non-blocking logging with rate limiting for high error rates
  // Implement error rate limiting to prevent log spam during error storms
  const errorKey = `${error.name}_${error.code || 'unknown'}`;
  const now = Date.now();
  
// Initialize module-level rate limiting with proper scoping
let errorRateLimit = null;
let errorRateLimitOrder = null;
let maxErrorRateLimit = 100;
let maxErrorCounts = 200;
let errorRateLimitLock = false;
let errorCountsLock = false;
let errorCounts = null;
let errorCountsOrder = null;

// Initialize on first use
function initializeRateLimiting() {
  if (!errorRateLimit) {
    errorRateLimit = new Map();
    errorRateLimitOrder = []; // Track insertion order for LRU eviction
    errorCounts = new Map();
    errorCountsOrder = []; // Track order for LRU eviction
  }
}

// Enhanced rate limiting for identical errors with atomic operations
  initializeRateLimiting();
  
  const lastLogged = errorRateLimit.get(errorKey);
  if (!lastLogged || (now - lastLogged) > 60000) {
    logAsync('error', errorLog);
    errorRateLimit.set(errorKey, now);
    
    // Atomic update of error rate limit
    if (!errorRateLimitLock) {
      errorRateLimitLock = true;
      try {
        errorRateLimit.set(errorKey, now);
        
        // Efficient LRU eviction using array tracking
        const existingIndex = errorRateLimitOrder.indexOf(errorKey);
        if (existingIndex === -1) {
          // New error key - add to order
          errorRateLimitOrder.push(errorKey);
          
          // Enforce size limit with batch eviction for better performance
          if (errorRateLimitOrder.length > maxErrorRateLimit) {
            const excess = errorRateLimitOrder.length - maxErrorRateLimit;
            const evictedKeys = errorRateLimitOrder.splice(0, excess);
            
            // Batch delete evicted keys
            evictedKeys.forEach(key => errorRateLimit.delete(key));
          }
        } else {
          // Move to end (most recently used)
          errorRateLimitOrder.splice(existingIndex, 1);
          errorRateLimitOrder.push(errorKey);
        }
      } finally {
        errorRateLimitLock = false;
      }
    } else {
      // Skip logging but count the error for metrics
      if (!errorCounts) {
        errorCounts = new Map();
        errorCountsOrder = []; // Track order for LRU eviction
      }
      
      // Atomic update of error counts
      if (!errorCountsLock) {
        errorCountsLock = true;
        try {
          errorCounts.set(errorKey, (errorCounts.get(errorKey) || 0) + 1);
          
          // Efficient LRU eviction for error counts
          const existingCountIndex = errorCountsOrder.indexOf(errorKey);
          if (existingCountIndex === -1) {
            // New error key - add to order
            errorCountsOrder.push(errorKey);
            
            // Enforce size limit with batch eviction for better performance
            if (errorCountsOrder.length > maxErrorCounts) {
              const excess = errorCountsOrder.length - maxErrorCounts;
              const evictedKeys = errorCountsOrder.splice(0, excess);
              
              // Batch delete evicted keys
              evictedKeys.forEach(key => errorCounts.delete(key));
            }
          } else {
            // Move to end (most recently used)
            errorCountsOrder.splice(existingCountIndex, 1);
            errorCountsOrder.push(errorKey);
          }
        } finally {
          errorCountsLock = false;
        }
      } else {
        // If locked, just increment the count without LRU operations
        errorCounts.set(errorKey, (errorCounts.get(errorKey) || 0) + 1);
      }
    }
        } else {
      ...
      // If locked, just update the timestamp without LRU operations
    ...
    if (!this.errorCounts) {
      ...
      this.errorCountsOrder = []; // Track order for LRU eviction
    }
    
    // Atomic update of error counts
    if (!this.errorCountsLock) {
      this.errorCountsLock = true;
      try {
        this.errorCounts.set(errorKey, (this.errorCounts.get(errorKey) || 0) + 1);
        
// Efficient LRU eviction for error counts
    try {
      const existingCountIndex = this.errorCountsOrder.indexOf(errorKey);
      if (existingCountIndex === -1) {
        // New error key - add to order
        this.errorCountsOrder.push(errorKey);
        
        // Enforce size limit with batch eviction for better performance
        if (this.errorCountsOrder.length > maxErrorCounts) {
          const excess = this.errorCountsOrder.length - maxErrorCounts;
          const evictedKeys = this.errorCountsOrder.splice(0, excess);
          
          // Batch delete evicted keys
          evictedKeys.forEach(key => this.errorCounts.delete(key));
        }
      } else {
        // Move to end (most recently used)
        this.errorCountsOrder.splice(existingCountIndex, 1);
        this.errorCountsOrder.push(errorKey);
      }
    } finally {
      this.errorCountsLock = false;
    }
  } else {
    // If locked, just increment the count without LRU operations
    this.errorCounts.set(errorKey, (this.errorCounts.get(errorKey) || 0) + 1);
    ...
    if (existingCountIndex === -1) {
      ...
      // Enforce size limit with batch eviction
      if (this.errorCountsOrder.length > this.maxErrorCounts) {
        const excess = this.errorCountsOrder.length - this.maxErrorCounts;
        const evictedKeys = this.errorCountsOrder.splice(0, excess);
  ...
  // Schedule AI analysis in background with timeout protection and rate limiting
  setImmediate(() => {
    // Rate limit AI analysis during error storms to prevent resource exhaustion
    const analysisKey = `${error.name}_${error.code || 'unknown'}`;
    const now = Date.now();
    
    if (!this.analysisRateLimit) {
      this.analysisRateLimit = new Map();
      this.analysisRateLimitOrder = []; // Track insertion order for LRU eviction
    }
    
    const lastAnalyzed = this.analysisRateLimit.get(analysisKey);
    if (lastAnalyzed && (now - lastAnalyzed) < 30000) {
      // Skip analysis for recent identical errors (30 second cooldown)
      return;
    }
    
    this.analysisRateLimit.set(analysisKey, now);
    
    // Track order for LRU eviction (max 50 entries to prevent memory bloat)
    if (!this.analysisRateLimitOrder.includes(analysisKey)) {
      this.analysisRateLimitOrder.push(analysisKey);
      if (this.analysisRateLimitOrder.length > 50) {
        const oldestKey = this.analysisRateLimitOrder.shift();
        this.analysisRateLimit.delete(oldestKey);
      }
    }
    ...
    scheduleAnalysis(error, contextString, analyzeError)
      ...
      .catch(analysisErr => {
        // Rate limit analysis error logging as well
        const errorLogKey = 'analysis_error';
        const lastErrorLogged = this.errorRateLimit ? this.errorRateLimit.get(errorLogKey) : 0;
        if (!lastErrorLogged || (now - lastErrorLogged) > 60000) {
          logAsync('error', analysisErr);
          if (this.errorRateLimit) {
            this.errorRateLimit.set(errorLogKey, now);
          }
        }
      });
  });
...
/**
 * Initialize error rate limiting cleanup to prevent memory leaks
 * 
 * Sets up periodic cleanup of expired error rate limit entries and
 * ensures the error tracking maps don't grow unbounded over time.
 */
function initializeErrorRateLimitCleanup() {
  // Set up periodic cleanup every 5 minutes
  const cleanupInterval = setInterval(() => {
    cleanupErrorRateLimits();
  }, 300000); // 5 minutes
  
  // Unref the interval to allow clean process exit
...
/**
 * Clean up expired error rate limit entries
 * 
 * Removes entries that have expired beyond the rate limiting window
 * and enforces LRU size limits to prevent memory growth.
 */
function cleanupErrorRateLimits() {
  const now = Date.now();
  const rateLimitWindow = 60000; // 1 minute window
  
  try {
    // Clean up error rate limit map with lock protection
    if (errorRateLimit && errorRateLimitOrder && !errorRateLimitLock) {
      errorRateLimitLock = true;
      try {
        const expiredKeys = [];
        
        // Find expired entries
        for (const [key, timestamp] of errorRateLimit.entries()) {
        ...
        expiredKeys.forEach(key => {
          ...
          const orderIndex = errorRateLimitOrder.indexOf(key);
          if (orderIndex !== -1) {
            errorRateLimitOrder.splice(orderIndex, 1);
          }
        });
    ...
    // Clean up error counts map with lock protection
    if (errorCounts && errorCountsOrder && !errorCountsLock) {
      errorCountsLock = true;
      try {
        // Only enforce size limit for error counts (keep them longer for metrics)
        if (errorCountsOrder.length > maxErrorCounts) {
          const excess = errorCountsOrder.length - maxErrorCounts;
          const evictedKeys = errorCountsOrder.splice(0, excess);
          evictedKeys.forEach(key => errorCounts.delete(key));
    ...
    // Ignore cleanup errors to prevent infinite loops
    console.error('Error during error rate limit cleanup:', error.message);
  }
}
...
let cleanupInterval;
try {
  cleanupInterval = initializeErrorRateLimitCleanup();
} catch (error) {
  console.error('Failed to initialize error rate limit cleanup:', error.message);
}

// Export cleanup functions for testing
module.exports.cleanupErrorRateLimits = cleanupErrorRateLimits;
module.exports.initializeErrorRateLimitCleanup = initializeErrorRateLimitCleanup;