...
const qerrors=(error,location,context={})=>{try{const handler=getScalableErrorHandler();return handler.handleError(error,location,context);}catch(handlingError){console.error('qerrors failed to handle error:',handlingError.message);console.error('Original error:',error.message);error&&error.stack&&console.error(error.stack);}};

const qerrorsMiddleware=(options={})=>{const handler=getScalableErrorHandler();return(error,req,res,next)=>{try{const context=extractContext(error,{url:req.url,method:req.method,userAgent:req.get('User-Agent'),ip:req.ip||req.connection.remoteAddress});handler.handleError(error,'express.middleware',context).then(result=>{req.accepts('html')?sendHtmlError(res,error,result):sendJsonError(res,error,result);}).catch(middlewareError=>{console.error('Middleware error:',middlewareError);sendFallbackError(res,error);});}catch(middlewareError){console.error('Express middleware failed:',middlewareError.message);sendFallbackError(res,error);}};};
...
const sendFallbackError=(res,error)=>{const errorId=generateErrorId();res.get('Content-Type')&&res.get('Content-Type').includes('html')?res.status(500).send(`Error ID: ${errorId}`):res.status(500).json({error:'Internal Server Error',errorId});};
