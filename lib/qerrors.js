'use strict';
const{randomUUID}=require('crypto'),{ScalableErrorHandler}=require('./scalabilityFixes'),{getQueueRejectCount,getQueueLength}=require('./qerrorsQueue'),{clearAdviceCache,purgeExpiredAdvice,startAdviceCleanup,stopAdviceCleanup}=require('./qerrorsCache'),escapeHtml=require('escape-html');
let scalableErrorHandler,shutdownListenersAdded,cleanupInterval;
...
const extractContext=(error,context={})=>{try{const safeContext={timestamp:new Date().toISOString(),errorType:error.constructor.name,message:error.message,stack:error.stack?.split('\n').slice(0,10)||[],...context};safeContext.password&&delete safeContext.password;safeContext.token&&delete safeContext.token;safeContext.apiKey&&delete safeContext.apiKey;return safeContext;}catch(extractionError){console.error('Failed to extract error context:',extractionError.message);return{timestamp:new Date().toISOString(),extractionFailed:true};}};
const qerrors=(error,location,context={})=>{try{const handler=getScalableErrorHandler();return handler.handleError(error,location,context);}catch(handlingError){console.error('qerrors failed to handle error:',handlingError.message);error?.message&&console.error('Original error:',error.message);error?.stack&&console.error(error.stack);return null;}};
const qerrorsMiddleware=(options={})=>{const handler=getScalableErrorHandler();
return(error,req,res,next)=>{try{const context=extractContext(error,{url:req.url,method:req.method,userAgent:req.get('User-Agent'),ip:req.ip||req.connection.remoteAddress});
handler.handleError(error,'express.middleware',context).then(result=>req.accepts('html')?sendHtmlError(res,error,result):sendJsonError(res,error,result)).catch(middlewareError=>{console.error('Middleware error:',middlewareError);sendFallbackError(res,error);}).finally(()=>next?.());
}catch(middlewareError){console.error('Express middleware failed:',middlewareError.message);sendFallbackError(res,error);next?.());}};
...
module.exports.getQueueStats=()=>({length:getQueueLength(),rejectCount:getQueueRejectCount()});
module.exports.getAnalysisCache=()=>({clear:clearAdviceCache,purgeExpired:purgeExpiredAdvice,startCleanup:startAdviceCleanup,stopCleanup:stopAdviceCleanup});