...
async function analyzeError(error, contextString) {
        ...
        if (!process.env.OPENAI_API_KEY) { //(graceful degradation when API token unavailable)
                if (!warnedMissingToken) { //(check if warning already logged to avoid console spam)
                        console.error(`Missing OPENAI_API_KEY in environment variables.`); //(inform developer about missing token)
...
Stack: ${truncatedStack}`; //(JSON format prompt for structured response with explicit instruction)
        ...
        // Try LangChain-based analysis first (preferred method for model flexibility)
        try {
                const aiManager = getAIModelManager();
                const advice = await aiManager.analyzeError(errorPrompt);
                
                if (advice) {
                        verboseLog(`qerrors is returning LangChain advice for
                ...
                verboseLog(`LangChain analysis failed: ${langchainError.message}, falling back to axios method`);
        }

        // Fallback to original axios-based OpenAI API call for backward compatibility
        ...
        const openaiBody = { //(payload for OpenAI API structured for optimal error analysis)
                model: config.getEnv('QERRORS_AI_MODEL', 'gpt-4o'), //(configurable model selection)
        ...
        if (advice && typeof advice === 'object') { //(validate response structure and handle different API response formats with defensive programming)
                verboseLog(`qerrors is returning axios fallback advice for
                                the error name: "${error.uniqueErrorName}",
                                with the error message: "${error.message}",
