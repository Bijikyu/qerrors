 ...
 */
const verboseLog = msg => config.getEnv('QERRORS_VERBOSE') === 'true' && console.log(msg);
...
const stopAdviceCleanup = () => { cleanupHandle && (clearInterval(cleanupHandle), cleanupHandle = null); };

const logQueueMetrics = () => logger.then(l => l.info(`metrics queueLength=${getQueueLength()} queueRejects=${getQueueRejectCount()}`));
...
const stopQueueMetrics = () => { metricHandle && (clearInterval(metricHandle), metricHandle = null); };
...
const clearAdviceCache = () => { adviceCache.clear(); adviceCache.size === 0 && stopAdviceCleanup(); };
