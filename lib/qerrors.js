...
async function qerrors(error, context, req, res, next) {
  ...
  // Simple rate limiting for identical errors (allow max 1 per minute)
  if (!this.errorRateLimit) {
    this.errorRateLimit = new Map();
    this.errorRateLimitOrder = []; // Track insertion order for LRU eviction
  }
  
  const lastLogged = this.errorRateLimit.get(errorKey);
  if (!lastLogged || (now - lastLogged) > 60000) {
    logAsync('error', errorLog);
    this.errorRateLimit.set(errorKey, now);
    
    // Track order for LRU eviction (max 100 entries)
    if (!this.errorRateLimitOrder.includes(errorKey)) {
      this.errorRateLimitOrder.push(errorKey);
      // Enforce size limit to prevent memory leaks
      if (this.errorRateLimitOrder.length > 100) {
        const oldestKey = this.errorRateLimitOrder.shift();
        this.errorRateLimit.delete(oldestKey);
      }
    }
  } else {
    ...
    // Enforce size limit on error counts (max 200 entries)
    if (!this.errorCountsOrder.includes(errorKey)) {
      this.errorCountsOrder.push(errorKey);
      if (this.errorCountsOrder.length > 200) {
        const oldestKey = this.errorCountsOrder.shift();
