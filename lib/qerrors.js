...
async function scheduleAnalysis(err, ctx) { //limit analyzeError concurrency
        ...
        run.finally(() => { if (limit.activeCount === 0 && limit.pendingCount === 0) stopQueueMetrics(); }); //(stop metrics when queue fully drained to prevent unnecessary interval overhead in idle periods)
...
async function analyzeError(error, context) {
        ...
        and only the error. Do not use blank lines.`; //(carefully crafted prompt optimizes for practical debugging advice, prevents generic responses and formatting issues)
        ...
        const openaiBody = { //(payload for OpenAI API structured for optimal error analysis)
                model: 'gpt-4.1', //(latest model for best analysis quality and accuracy)
        ...
        if (advice && typeof advice === 'object') { //(validate response structure and handle different API response formats with defensive programming)
                ...
                //(handle structured response with data property for consistent access patterns)
                if (advice.data) {
