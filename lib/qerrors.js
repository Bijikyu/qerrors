 ...
 */
function extractContext(error, context = {}) {
  try {
    const safeContext = {
 ...
 */
function qerrors(error, location, context = {}) {
  try {
 ...
 */
function qerrorsMiddleware(options = {}) {
  const handler = getScalableErrorHandler();
 ...
 */
function sendHtmlError(res, error, analysis) {
 ...
 */
function sendJsonError(res, error, analysis) {
  res.status(500).json({
