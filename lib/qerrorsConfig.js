...
// Dynamic concurrency and queue limits based on system resources
const os = require('os');
const cpuCount = os.cpus().length;
const totalMemory = os.totalmem();
const availableMemory = os.freemem();

// Calculate optimal concurrency based on CPU cores and memory
const cpuBasedConcurrency = Math.max(5, Math.min(50, cpuCount * 3));
const memoryBasedConcurrency = Math.max(5, Math.min(30, Math.floor(availableMemory / (100 * 1024 * 1024)))); // 100MB per concurrent task
...
const memoryBasedQueue = Math.max(100, Math.min(2000, Math.floor(availableMemory / (10 * 1024 * 1024)))); // 10MB per queued item
const cpuBasedQueue = cpuCount * 100;
const dynamicQueue = Math.min(memoryBasedQueue, cpuBasedQueue);

// Load and validate with dynamic defaults
const rawConc = config.getInt('QERRORS_CONCURRENCY', dynamicConcurrency);
const rawQueue = config.getInt('QERRORS_QUEUE_LIMIT', dynamicQueue);
const SAFE_THRESHOLD = config.getInt('QERRORS_SAFE_THRESHOLD', Math.max(2000, dynamicQueue * 2));

// Apply safety clamping with dynamic limits
