...
class SimpleMemoryMonitor {
  ...
  checkMemoryUsage() {
    ...
    return heapUsedPercent > this.criticalPercent ? (this.isHighMemoryUsage = true, { level: 'critical', percent: heapUsedPercent }) : heapUsedPercent > this.warningPercent ? (this.isHighMemoryUsage = true, { level: 'warning', percent: heapUsedPercent }) : (this.isHighMemoryUsage = false, { level: 'normal', percent: heapUsedPercent });
  }
  ...
  start() {
    this.interval = setInterval(() => this.checkMemoryUsage(), this.checkInterval * 2);
  }
...
const getMemoryAwareQueueLimit = () => {
  const currentPressure = memoryMonitor.checkMemoryUsage().level;
...
const logQueueMetrics = () => {
  try {
    ...
    console.log(`metrics queueLength=${limit.pendingCount} queueRejects=${managerGetRejectCount()} memoryPressure=${memoryStats.level} memoryLimit=${memoryAwareLimit} heap_used=${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)}MB`);
  } catch (error) {
...
module.exports = {
  ...
  getMemoryStats: () => memoryMonitor.checkMemoryUsage(),
  cleanupTimers
};