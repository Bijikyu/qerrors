...
const createStackHash = (stack) => {
  ...
  // Extract only the first few lines of stack trace for hashing
...
class BoundedTimerSet {
  ...
  add(timer) {
    ...
    this.timerMetadata.set(timer, { added: timestamp, accessCount: 1 });
    return true;
  }
  ...
  cleanup() {
    const now = Date.now();
    const expiredTimers = [];
    const maxAge = 300000; // 5 minutes max age for timers
    ...
    if (expiredTimers.length > 10 && global.gc) {
      global.gc();
    }
  }
...
const activeTimers = new BoundedTimerSet(500); // Limit to 500 active timers
...
async function scheduleAnalysis(err, ctx, analyzeErrorFn) {
  ...
  const ABSOLUTE_MAX_QUEUE_SIZE = 500; // Hard limit to prevent memory exhaustion
