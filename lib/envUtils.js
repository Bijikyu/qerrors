...
const getMissingEnvVars = varArr => {
  // Log each missing variable for debugging and monitoring
  varArr.forEach(name => {
    if (!process.env[name]) {
      logError(new Error(`Missing environment variable: ${name}`), 'envUtils.getMissingEnvVars', {
  ...
  // Return array of variables that are not set in process.env
