...
let dotenvLoaded=false;const loadDotenv=async()=>{if(!dotenvLoaded){try{require('dotenv').config();dotenvLoaded=true;}catch(error){try{const qerrors=require('./qerrors');qerrors(error,'envUtils.loadDotenv',{operation:'dotenv_loading'});}catch(qerrorsError){console.error('Failed to log qerrors:',qerrorsError.message);}console.warn('Failed to load .env file:',error.message);dotenvLoaded=true;}}};let envFileExistsCache=null;const checkEnvFileExists=async()=>{if(envFileExistsCache===null){try{const fs=require('fs').promises;await fs.access('.env');envFileExistsCache=true;}catch(error){envFileExistsCache=false;}}return envFileExistsCache;};
 ...
 */
const getMissingEnvVars=varArr=>{try{const qerrors=require('./qerrors');varArr.forEach(name=>{if(!process.env[name]){try{qerrors(new Error(`Missing environment variable: ${name}`),'envUtils.getMissingEnvVars',{operation:'environment_variable_check',variableName:name});}catch(qerrorsError){console.error('Failed to log qerrors:',qerrorsError.message);}}});return varArr.filter(name=>!process.env[name]);}catch(error){console.error('Error in getMissingEnvVars:',error.message);return varArr.filter(name=>!process.env[name]);}};
