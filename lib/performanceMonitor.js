...
const { UnifiedCircularBuffer } = require('./shared/dataStructures');
...
class PerformanceMonitor extends EventEmitter {
  constructor(options = {}) {
    ...
    this.metrics = {
      blockingOperations: new UnifiedCircularBuffer(25),
      memorySnapshots: new UnifiedCircularBuffer(20),
      gcPauses: new UnifiedCircularBuffer(10),
      eventLoopLag: new UnifiedCircularBuffer(25),
      cpuIntensiveOperations: new UnifiedCircularBuffer(10),
      alerts: new UnifiedCircularBuffer(10)
    };
...
/**
   ...
   * Begins collecting metrics at configured intervals and emits alerts for threshold violations
   */
  start() {
...
/**
   ...
   * Clears the monitoring interval and cleans up resources
   */
  stop() {
    if (this.monitoringInterval) {
...
/**
   * Monitor a potentially blocking operation for performance analysis
   * @param {string} name - Operation name for tracking
   * @param {Function} operation - Operation to monitor
   * @returns {Function} Function to end monitoring and report results
   */
...
/**
 * Collect current metrics with reduced overhead
 */
collectMetrics() {
  ...
  // Measure event loop lag less frequently
...
/**
 * Check thresholds and create alerts with simplified logic
 */
checkThresholds() {
    // Check memory growth with reduced computation
    const snapshots = this.metrics.memorySnapshots;
    if (snapshots.length >= 2) {
      const snapshotArray = snapshots.toArray();
      const oldest = snapshotArray[0];
      const newest = snapshotArray[snapshotArray.length - 1];
    ...
    // Simplified blocking operations check
    const now = Date.now();
    const recentBlocking = this.metrics.blockingOperations.filter(op => 
      op.blocking && (now - op.timestamp < 60000)
    );
