...
class PrivacyManager {
  constructor() {
    ...
    this.maxConsentRecords = parseInt(process.env.MAX_CONSENT_RECORDS) || 10000; // Memory limit
    ...
    // Start cleanup interval for expired records
    this.startCleanupInterval();
  }

/**
 * Start cleanup interval for expired consent records
 */
...
/**
 * Clean up expired consent records to prevent memory leaks
 */
cleanupExpiredRecords() {
  ...
  for (const [key, record] of this.consentRecords.entries()) {
    ...
    // Delete if record is too old or has been withdrawn for over 7 years
    if (recordAge > maxAge || 
  ...
  // If we're still over the memory limit, remove oldest records
  if (this.consentRecords.size > this.maxConsentRecords) {
    const records = Array.from(this.consentRecords.entries());
    // Sort by timestamp (oldest first)
    records.sort((a, b) => new Date(a[1].timestamp).getTime() - new Date(b[1].timestamp).getTime());
    
    const excessCount = this.consentRecords.size - this.maxConsentRecords;
    for (let i = 0; i < excessCount; i++) {
      this.consentRecords.delete(records[i][0]);
      deletedCount++;
    }
  }
  
  if (deletedCount > 0) {
    console.log(`Cleaned up ${deletedCount} expired consent records`);
  }
}
 ...
 */
  ...
  encryptData(data) {
    try {
      ...
      try {
        ...
        qerrors(error, 'privacyManager.encryptData.encryption', {
      ...
      qerrors(error, 'privacyManager.encryptData', {
        operation: 'sensitive_data_encryption',
        hasData: !!data,
        dataType: typeof data
      });
      throw error;
  ...
  decryptData(encryptedData) {
    try {
      ...
      try {
        ...
        qerrors(error, 'privacyManager.decryptData.decryption', {
      ...
      qerrors(error, 'privacyManager.decryptData', {
        operation: 'sensitive_data_decryption',
        hasEncryptedData: !!encryptedData
      });
      throw error;
    }
  ...
  recordConsent(userId, consentData) {
    ...
    if (this.consentRecords.size >= this.maxConsentRecords) {
      // Trigger cleanup to make space
      this.cleanupExpiredRecords();
      
      // If still at limit, reject new consent
      if (this.consentRecords.size >= this.maxConsentRecords) {
        console.warn(`Consent records at memory limit (${this.maxConsentRecords}), rejecting new consent record`);
        throw new Error('Unable to store consent record - memory limit reached');
      }
    }
 ...
 * Optimized to reduce hash operations and lookups
 */
getConsentByUserId(userId) {
  // First check if this looks like a hashed ID (16-char hex string)
  const isLikelyHashed = /^[a-f0-9]{16}$/i.test(userId);
  
  if (isLikelyHashed) {
    // If it looks like a hashed ID, try that first (most common case)
    let consent = this.consentRecords.get(userId);
    if (consent) {
      return consent;
    }
    
    // Fallback to original ID check (for backward compatibility)
    return this.consentRecords.get(userId);
  } else {
    // If it's an original ID, try that first
    let consent = this.consentRecords.get(userId);
    if (consent) {
      return consent;
    }
    
    // Only compute hash if original ID lookup failed
    ...
    if (hashedUserId && hashedUserId.value) {
      ...
      // Optional: Store the mapping for faster future lookups
      if (consent) {
