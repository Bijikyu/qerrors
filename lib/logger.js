...
function sanitizeContext(context, level = 'INFO') { //sanitize context objects recursively
    ...
    if (Array.isArray(context)) {
        return context.map(item => sanitizeContext(item, level)); //recursively sanitize array items
    }
    ...
    Object.keys(context).forEach(key => {
        ...
        const sensitiveKeys = ['password', 'token', 'key', 'secret', 'auth', 'credential', 'card', 'ssn', 'cvv'];
