...
const getInt = (name, defaultValOrMin, min) => {
  const envValue = process.env[name], int = parseInt(envValue || '', 10), moduleDefault = typeof defaults[name] === 'number' ? defaults[name] : parseInt(defaults[name] || '0', 10);
  let fallbackVal, minVal;
  if (arguments.length <= 1) { fallbackVal = moduleDefault; minVal = 1; } else if (arguments.length === 2) { fallbackVal = moduleDefault; minVal = typeof defaultValOrMin === 'number' ? defaultValOrMin : 1; } else { fallbackVal = typeof defaultValOrMin === 'number' ? defaultValOrMin : moduleDefault; minVal = typeof min === 'number' ? min : 1; }
...
const validateRequiredVars = varNames => {
  ...
  return { isValid: missing.length === 0, missing, present: varNames.filter(name => process.env[name]) };
};
