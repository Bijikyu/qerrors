 ...
 */
function getEnv(name, defaultVal) { //return env var or default when undefined
 ...
 */
function getInt(name, defaultValOrMin, min) { //parse env integer with minimum enforcement
  const envValue = process.env[name]; //(get raw env value)
  const int = parseInt(envValue || '', 10); //attempt parse
  const moduleDefault = typeof defaults[name] === 'number' ? defaults[name] : parseInt(defaults[name], 10); //(get module default safely)
