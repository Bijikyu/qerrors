...
class DistributedRateLimiter {
  ...
  cleanupFallbackCache() {
     ...
     let totalEntries = 0;
     const maxEntries = 1000; // Hard limit to prevent unbounded growth
     
     const cacheEntries = Array.from(this.fallbackCache.entries());
     for (const [key, data] of cacheEntries) {
       ...
       if (totalEntries > maxEntries) {
         ...
         const entriesToRemove = Array.from(this.fallbackCache.keys()).slice(0, totalEntries - maxEntries);
         for (const removeKey of entriesToRemove) {
           this.fallbackCache.delete(removeKey);
         }
