...
module.exports = {
  ...
  // Enhanced error handling functions (from TypeScript implementation)
  attempt: async (fn) => {
    try {
      ...
      try {
        ...
        // Note: qerrors not imported here to avoid circular dependency
        console.error('attempt error:', errorObj);
      } catch (qerror) {
  ...
  executeWithQerrors: async (options) => {
    try {
      ...
      try {
        const errorObj = error instanceof Error ? error : new Error(String(error));
        // Note: qerrors not imported here to avoid circular dependency
        console.error('executeWithQerrors error:', errorObj);
      } catch (qerror) {
