 ...
 */
module.exports={ServiceError,errorUtils,safeUtils,createTypedError,createStandardError,ErrorTypes,ErrorSeverity,ERROR_STATUS_MAP,ERROR_SEVERITY_MAP,ErrorFactory,handleSimpleError,errorMiddleware,attempt:async fn=>{try{const value=await fn();return{ok:true,value};}catch(error){try{const errorObj=error instanceof Error?error:new Error(String(error));console.error('attempt error:',errorObj);}catch(qerror){console.error('qerrors logging failed in attempt',qerror);}return{ok:false,error};}},executeWithQerrors:async options=>{try{return await options.operation();}catch(error){console.error(options.failureMessage,error);try{const errorObj=error instanceof Error?error:new Error(String(error));console.error('executeWithQerrors error:',errorObj);}catch(qerror){console.error('qerrors logging failed in executeWithQerrors',qerror);}throw error;}}};