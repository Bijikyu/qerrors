...
/**
 * Error Types and Handling System - Comprehensive Error Management
 * 
 * Purpose: Provides a complete error handling system with standardized error types,
 * severity levels, HTTP status mapping, and factory functions for creating
 * consistent error objects. This system ensures uniform error handling across
 * the entire application.
 * 
 * Design Rationale:
 * - Standardization: Consistent error format and handling across all modules
 * - HTTP integration: Automatic mapping between error types and HTTP status codes
 * - Severity tracking: Categorization of errors by impact and urgency
 * - Factory patterns: Convenient creation of common error types
 * - Express integration: Built-in middleware for web application error handling
 * - Context preservation: Maintains error context and chain of causality
 * 
 * Key Features:
 * - ServiceError class with enhanced properties and JSON serialization
 * - Error type enumeration with automatic HTTP status mapping
 * - Severity classification for monitoring and alerting
 * - Factory functions for common error scenarios
 * - Safe utilities that never throw exceptions
 * - Express middleware for standardized error responses
 */

// Import error type definitions and mappings from configuration
const localVars = require('../config/localVars');
const { 
  ErrorTypes,           // Enumeration of standardized error types
  ErrorSeverity,        // Enumeration of error severity levels
  ERROR_STATUS_MAP,     // Mapping from error types to HTTP status codes
  ERROR_SEVERITY_MAP    // Mapping from error types to severity levels
} = localVars;
