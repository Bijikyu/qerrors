...
const rotationOpts = ROTATION_OPTS;

const { LOG_DIR, DISABLE_FILE_LOGS } = localVars;
const logDir = LOG_DIR;
let disableFileLogs = DISABLE_FILE_LOGS;
...
async function buildLogger() {
  await initLogDir();
  const maxDays = Number(localVars.QERRORS_LOG_MAX_DAYS) || 0;
  disableFileLogs = disableFileLogs || !!localVars.QERRORS_DISABLE_FILE_LOGS;
