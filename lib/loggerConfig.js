...
let disableFileLogs = DISABLE_FILE_LOGS;  // Flag to disable file logging (can be modified at runtime)
  ...
  const log = createLogger({
    ...
    transports: (() => {
      ...
      if (localVars.QERRORS_VERBOSE !== 'false') {
        transports.push(new transports.Console({ format: consoleFormat }));
      }
