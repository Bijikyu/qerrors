...
/**
 * Create a concurrency limiter using p-limit library with adaptive scaling
 * 
 ...
 * - Adaptive scaling: Dynamically adjusts based on system load
 * 
 * @param {number} max - Maximum number of concurrent operations allowed
 * @returns {Function} Enhanced concurrency limiter function with adaptive scaling
 * 
 * Example:
...
const createLimiter = (max) => {
  ...
  return async (task) => {
    ...
    try {
      ...
      // Adaptive scaling: if processing is fast, we could allow more concurrency
      // This is a placeholder for future adaptive scaling logic
      if (averageProcessingTime < 100 && activeCount < max) {
        // Processing is fast, could potentially increase limits
      }
