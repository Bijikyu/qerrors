...
const MAX_QUEUE_SIZE = 200;      // Maximum queue size to prevent memory exhaustion
...
const createLimiter = (max) => {
  ...
  return async (task) => {
    ...
    if (queueSize >= MAX_QUEUE_SIZE) {
      queueRejectCount++;
      throw new Error('Queue at capacity - request rejected to prevent memory exhaustion');
    }
