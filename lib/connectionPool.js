...
class ConnectionPool {
  ...
  async closeIdleConnections() {
    ...
    // Find idle connections to close - use bounded iteration with time limit
    const startTime = process.hrtime.bigint();
    const maxIterationTime = 10000000; // 10ms max for iteration
    let iterationCount = 0;
    const maxIterations = Math.min(this.pool.size, 1000); // Hard cap of 1000 iterations
    
    for (const [id, conn] of this.pool.entries()) {
      iterationCount++;
      
      // Check time limit to prevent blocking
      if (iterationCount > maxIterations || (process.hrtime.bigint() - startTime) > maxIterationTime) {
        break;
      }
