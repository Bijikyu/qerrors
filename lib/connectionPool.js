...
// Use shared implementations to eliminate duplication
const { BoundedSet, BoundedLRUCache } = require('./shared/BoundedSet');
...
class ConnectionPool {
  constructor(options = {}) {
    ...
    this.waitingQueue = new BoundedQueue(
      Math.max(this.max * 2, 50), // Ensure minimum queue size: max 2x connections or 50, whichever is larger
      Math.max(5, Math.floor(availableMemory / (200 * 1024 * 1024))) // Reduced to 2.5% of available memory
    );
  ...
  async initializePool() {
    try {
      ...
      // Bounded array for initialization promises to prevent memory bloat
      const initPromises = [];
      const maxInitPromises = Math.min(this.min, 10); // Cap at 10 concurrent initializations
      
      for (let i = 0; i < maxInitPromises; i++) {
  ...
  async closeIdleConnections() {
    const now = Date.now();
    // Bounded array for connections to close - limit batch size
    const connectionsToClose = [];
    const maxCloseBatch = 5; // Limit concurrent connection closures
    
    for (const [id, conn] of this.pool.entries()) {
      if (!conn.inUse && (now - conn.lastUsed) > this.idleTimeoutMillis) {
        // Keep minimum connections
        if (this.totalConnections > this.min && connectionsToClose.length < maxCloseBatch) {
...
/**
  ...
  /**
   * Concurrency limiter using p-limit module
   */
