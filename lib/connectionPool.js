...
class ConnectionPool {
  ...
  async initializePool() {
    // Create connections in parallel with controlled concurrency to avoid resource exhaustion
    const concurrencyLimit = Math.min(this.min, 5);
    const initPromises = [];
    
    for (let i = 0; i < this.min; i += concurrencyLimit) {
      const batch = Math.min(concurrencyLimit, this.min - i);
      const batchPromises = [];
      
      for (let j = 0; j < batch; j++) {
        batchPromises.push(this.createConnection());
      }
      
      initPromises.push(Promise.all(batchPromises));
    }
  ...
  /**
   * Close idle connections and clean up expired queue entries (optimized for scalability)
   */
  async closeIdleConnections() {
    ...
    // Close identified connections in parallel
    const closePromises = connectionsToClose.map(async (id) => {
