 ...
 * different projects, ensuring consistent logging behavior and avoiding heavy dependencies during test
...
/**
 * Initialize error logging for a module only outside of tests
 * Avoids pulling heavy CJS deps (langchain/langsmith/winston) during Jest runs
 * 
 * Purpose: Provides consistent module initialization with structured logging
 * Uses dynamic import to keep ESM parsers happy and avoid test overhead.
 * 
...
async function initializeModule(options = {}) {
  ...
  if (process.env.NODE_ENV === 'test') {
    return null; //(skip initialization during tests to avoid heavy deps)
  }
 ...
 * 
 * @returns {boolean} True if initialization should proceed, false for test environment
 */
function shouldInitialize() {
  return process.env.NODE_ENV !== 'test'; //(skip during tests)
}
...
function logModuleInit(moduleName, metadata = {}) {
  if (!shouldInitialize()) {
    return; //(skip logging during tests)
  }
