...
class RetryHandler {
  ...
  static async executeWithRetry(operation, options = {}) {
    const {
      retryAttempts = DEFAULT_ASYNC_CONFIG.retryAttempts,
      retryDelayMs = DEFAULT_ASYNC_CONFIG.retryDelayMs,
      retryBackoffMultiplier = DEFAULT_ASYNC_CONFIG.retryBackoffMultiplier,
      onRetry = null
    } = options;
