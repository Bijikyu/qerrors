...
/**
 * Create enhanced log entry with standardized format and optional memory tracking
 * 
 * Purpose: Creates comprehensive log entries that include standard logging information
 * plus memory usage metrics for higher-priority log levels. This provides valuable
 * debugging information for performance issues and resource monitoring.
 * 
 * Design Rationale:
 * - Memory monitoring: Automatically includes memory usage for warning and higher level logs
 * - Standardized format: Uses createEnhancedLogEntry for consistent log structure
 * - Performance awareness: Memory metrics help identify memory leaks and performance issues
 * - Level-based filtering: Only adds memory overhead for important log levels
 * - Unit conversion: Converts memory values to MB for human-readable output
 * 
 * @param {string} level - Log level (error, warn, info, debug, etc.)
 * @param {string} message - Log message
 * @param {Object} [context={}] - Additional context information
 * @param {string|null} [requestId=null] - Request ID for correlation tracking
 * @returns {Object} Enhanced log entry with optional memory metrics
 */
const createLogEntry = (level, message, context = {}, requestId = null) => {
  // Create base log entry using the enhanced log entry function
  const entry = createEnhancedLogEntry(level, message, context, requestId);

  // Add memory usage metrics for warning and higher priority log levels
  // This helps identify performance issues and memory leaks during critical events
  const levelConfig = LOG_LEVELS[level.toUpperCase()];
  
  if (levelConfig && levelConfig.priority >= LOG_LEVELS.WARN.priority) {
    const memUsage = process.memoryUsage();
    entry.memory = {
      heapUsed: Math.round(memUsage.heapUsed / 1024 / 1024),      // Convert bytes to MB
      heapTotal: Math.round(memUsage.heapTotal / 1024 / 1024),  // Convert bytes to MB
      external: Math.round(memUsage.external / 1024 / 1024),    // Convert bytes to MB
      rss: Math.round(memUsage.rss / 1024 / 1024)               // Convert bytes to MB
    };
  }
