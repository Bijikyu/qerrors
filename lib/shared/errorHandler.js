...
const safeQerrorsCall=(error,location,context={})=>{
  try{return qerrors(error,location,context);}
  catch(qerror){console.error('qerrors logging failed:',qerror.message);}
};

const safeAsyncQerrorsCall=async(error,location,context={})=>{
  try{return await qerrors(error,location,context);}
  catch(qerror){console.error('qerrors async logging failed:',qerror.message);}
};
...
const wrapFunctionWithErrorHandling=(fn,location)=>(...args)=>{
  try{return fn(...args);}
  catch(error){safeQerrorsCall(error,location,{args:args.slice(0,3)});throw error;}
};

const wrapAsyncFunctionWithErrorHandling=(fn,location)=>async(...args)=>{
  try{return await fn(...args);}
  catch(error){await safeAsyncQerrorsCall(error,location,{args:args.slice(0,3)});throw error;}
};
